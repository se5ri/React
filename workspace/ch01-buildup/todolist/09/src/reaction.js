...
const reaction = {
  ...
  useState: (initialValue) => {
    // 최초 호출 되었을 경우에만 초기값을 지정하고 이후에 다시 호출된 경우에는 이전 값을 유지한다
    // _stateValue: 이전값 , initialValue: 초기값
    // ??: null병합 연산자 , 왼쪽 피연산자가 null, undefined일 때 오른쪽 값을 사용
    ...
    function setValue(newValue) {
      ...
      // Object.is(a,b): adhk b가 같은지 여부를 반환
      // 객체일 때, 같은 메모리 주소를 가지고 있으면 true가 되므로 render() 호출 안 됨
      // 객체일 때, 객체 내부의 속성값이 바뀌었다고 해도 같은 메모리 주소를 가지고 있으면 true가 되므로 render 호출 안 됨
      // 원시형 타입일 때, 값만 다르면 false가 되므로 render() 호출됨
      // oldValue와 newValue 값이 같지 않으면 리렌더링 한다.
...
/* const a = [10];
// const a2 = a.slice();
const a2 = [...a]; // 전개연산자 (복사해서 새로운 배열을 만듦)
a2.push(20);
useState(a);
setValue(a2); // 같은 객체가 아니기 때문에 render가 호출 됨 */
