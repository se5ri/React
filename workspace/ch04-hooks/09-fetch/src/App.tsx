  ...
  */
  ...
  const fetchTodo = async (fetchParams: FetchParams) => {
    ...
    try {
      // 로딩 상태를 true로 설정
      setIsLoading(true);
      const res = await fetch(API_SERVER + fetchParams.url); // fetch는 promise를 반환한다 -> await 사용 가능
  ...
  return (
    <>
      ...
      <ul>{data?.items.map((item) => <li key={item._id}>{item.title}</li>)}</ul>
    </>
  );
}
